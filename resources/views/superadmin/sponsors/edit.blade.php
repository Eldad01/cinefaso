...
@section('page-title', 'Modifier le sponsor')

@section('content')
    <div class="max-w-2xl">
        <div class="rounded-xl border border-cf-line bg-cf-surface p-6">
            <form method="POST" action="{{ route('superadmin.sponsors.update', $sponsor) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
        ...
        <form method="POST" action="{{ route('superadmin.sponsors.destroy', $sponsor) }}" class="mt-4"
              onsubmit="return confirm('Supprimer définitivement ce sponsor ?');">
            @csrf
            @method('DELETE')
            <button type="submit" class="text-sm font-medium text-red-600 hover:text-red-700">
                <i class="ti ti-trash"></i> Supprimer ce sponsor
            </button>
        </form>
    </div>
@endsection
